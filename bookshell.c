...
struct command conversions1[] = {
    {"quit", quit},   {"q", quit},      {"exit", quit},   {"help", help},
    {"book", book},   {"bk", book},     {"shelf", shelf}, {"savef", savef},
    {"openf", openf}, {"showf", showf}, {"clean", clean}, {NULL, NULL}};

// Conversions for book argument
struct command bookConvert[] = {
    {"add", addBook}, {"show", showBooks}, {NULL, NULL}};

// Conversions for show argument
struct command shelfConvert[] = {
    {"add", addShelf}, {"show", showShelves}, {"rm", rmShelf}, {NULL, NULL}};
...
void main() {
    ...
    printf(
        "Welcome to \033[0;33mBookshell.c!\033[0m Type \"help\" for a list of "
...
void addBook() {
    ...
    if (*top != NULL && (*top)->bookName != NULL && (*top)->author)
        printf(
            "Successfully created book \033[0;33m%s\033[0m by "
            "\033[0;33m%s\033[0m\n",
...
void addShelf() {
    ...
    if (!(n > 0 && n < SHELVES)) {
        printf(
            "\033[0;31mERROR\033[0m create shelf failed: invalid shelf "
            "number\n");
        return;
    }
    ...
    if (shelves[n] != NULL) {
        printf(
            "A shelf already exists here, are you sure you want to overwrite "
            "it? Type 'overwrite' to continue, anything else to abort: ");
    ...
    bookshelf->bookName =
        (char *)malloc(sizeof(char) * (strlen(shelfName) + 1));
    ...
    if (shelves[n - 1] != NULL)
        printf(
            "Successfuly created shelf \033[0;33m%s\033[0m by "
            "\033[0;33m%s\033[0m in save slot %d\033[0m\n",
...
void showShelves() {
    for (int i = 0; i < SHELVES; i++) {
        if (shelves[i] != NULL) {
            struct stackNode *bottomNode = bottom(shelves[i]);
            if (bottomNode != NULL) {
                printf("Shelf %d: \033[0;33m%s\033[0m by \033[0;33m%s\033[0m\n",
                       i + 1, bottomNode->bookName, bottomNode->author);
            } else {
...
void showBooks() {
    ...
    if (n < SHELVES + 1 && n > 0) {
        if (top == NULL) {
            printf(
                "\033[0;31mERROR\033[0m show shelf %d failed: shelf slot "
                "empty\n",
                n);
            return;
        }
...
void quit() {
    ...
    for (int i = 0; i < SHELVES; i++) destroy(shelves[i]);
...
// Saves shelf state to a dynamically named .txt file, overwrites if file
// already exists
void savef() {
    if (!argsCheck(1)) return;
    ...
    if (userArgs[1][0] == '.') {
        printf(
            "\033[0;31mERROR\033[0m savef: file name cannot start with a "
            "'.'\n");
        return;
    }
    ...
    for (int i = 0; i < SHELVES; i++) reverse(&shelves[i]);
...
void openf() {
    ...
    if (userArgs[1][0] == '.') {
        printf(
            "\033[0;31mERROR\033[0m openf: cannot open files that start with a "
            "'.'\n");
        return;
    }
    ...
    int indexes[SHELVES];
    for (int i = 0; i < SHELVES; i++) fscanf(file, "%d ", &indexes[i]);

    char buffer[3];
    fgets(buffer, sizeof(buffer), file);
    for (int i = 0; i < SHELVES; i++) {
        if (indexes[i] != 0) shelves[i] = stack();
